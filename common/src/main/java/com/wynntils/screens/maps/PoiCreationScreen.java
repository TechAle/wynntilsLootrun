...
public final class PoiCreationScreen extends AbstractMapScreen implements TextboxScreen {
    ...
    protected void doInit() {
        ...
        this.addRenderableWidget(new Button.Builder(Component.literal("<"), (button) -> {
                    if (selectedIconIndex - 1 < 0) {
                        selectedIconIndex = PoiService.POI_ICONS.size() - 1;
                    } else {
                        selectedIconIndex--;
                    }
                })
                ...
                .build());
        this.addRenderableWidget(new Button.Builder(Component.literal(">"), (button) -> {
                    if (selectedIconIndex + 1 >= PoiService.POI_ICONS.size()) {
                        selectedIconIndex = 0;
                    } else {
                        selectedIconIndex++;
                    }
                ...
                .build());
        if (oldPoi != null && firstSetup) {
            int index = PoiService.POI_ICONS.indexOf(oldPoi.getIcon());
    ...
    public void doRender(PoseStack poseStack, int mouseX, int mouseY, float partialTick) {
        ...
        if (parsedXInput != null && parsedZInput != null) {
            Poi poi = new CustomPoi(
                    ...
                    PoiService.POI_ICONS.get(selectedIconIndex),
                    selectedVisiblity);
    ...
    private void renderIcon(PoseStack poseStack) {
        ...
        Texture texture = PoiService.POI_ICONS.get(selectedIconIndex);
